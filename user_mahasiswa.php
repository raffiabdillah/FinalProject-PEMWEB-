...
$cari = $_POST ['cr'];
if ($cari !=''){
$query = mysqli_query ($conn, "SELECT * FROM mahasiswa WHERE nama LIKE '%".$cari."%'
	OR NPM LIKE '%".$cari."%' OR alamat LIKE '%".$cari."%'");}else{
	$query = mysqli_query ($conn, "SELECT * FROM mahasiswa LIMIT $posisi,$batas");
}
...
<td><a onClick="return confirm('Yakin Ingin Menghapus?')" class="btn btn-danger" href="hapus_user_mahasiswa.php?id=<?php echo $data['npm'] ?>" role="button"><i class="fa fa-remove"></i>Hapus</a>
<form method="post" action="data_mahasiswa_cek.php">
<input type="hidden" name="cr" value="<?php echo $data['npm'] ?>" /></td>
</form>
</td>
</tr>
<?php }}else{
...
<?php
	$query2		= mysqli_query($conn, "SELECT * FROM mahasiswa");
...
<li class="page-item"><a class="page-link" style="color:blue">Halaman</a></li>
	...
	if ($i != $halaman){
		echo "<li class='page-item'><a class='page-link' href=\"user_mahasiswa.php?halaman=$i\">$i</a></li>";
	}
	else{
