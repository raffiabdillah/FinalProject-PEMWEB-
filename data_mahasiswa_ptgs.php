...
<head>
    ...
    <title>PETUGAS : DATA MAHASISWA</title>
...
?> 
    ...
    <form action="" method="post" class="tulisan_input2">
					<input type="text" name="cr" placeholder="Cari Data Mahasiswa" style="width:20%; padding:8px; margin-top:5px; margin-bottom:5px" class="form_input" />
    ...
    <table class="table1" align="center" width="100%">
		<tr>
            ...
            <th>Program Studi</th>
            <th>No Telepon</th>
            <th><a class="btn btn-dark" style="float:left;" href="tambah_data_mahasiswa_ptgs.php" role="button"><i class="fa fa-user-plus"></i>Tambah</a></th>
</tr>
<?php 
$batas =6;
...
$cari = $_POST ['cr'];
if ($cari !=''){
$query = mysqli_query ($conn, "SELECT * FROM data_mahasiswa WHERE NPM LIKE '%".$cari."%'
	OR nama LIKE '%".$cari."%' OR alamat LIKE '%".$cari."%'");}else{
	$query = mysqli_query ($conn, "SELECT * FROM data_mahasiswa LIMIT $posisi,$batas");
}
...
<td colspan="2">
<a class="btn btn-success" href="edit_data_mahasiswa.php?id=<?php echo $data['id_mahasiswa'] ?>" role="button"><i class="fa fa-pencil"></i> Edit</a>
<a onClick="return confirm('Yakin Ingin Menghapus?')" class="btn btn-danger" href="hapus_data_mahasiswa_ptgs.php?idd=<?php echo $data['id_mahasiswa'] ?>" role="button"><i class="fa fa-remove"></i>Hapus</a>
</form></td>
</tr>
<?php }}else{
...
<?php
	$query2		= mysqli_query($conn, "SELECT * FROM data_mahasiswa");
...
<li class="page-item"><a class="page-link" style="color:blue">Halaman</a></li>
	...
	if ($i != $halaman){
		echo "<li class='page-item'><a class='page-link' href=\"data_mahasiswa_ptgs.php?halaman=$i\">$i</a></li>";
	}
	else{
